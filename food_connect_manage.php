<html>
	...
	<body>
		...
		<?php
			...
			$pwd = $_POST['u_pwd'];
			//搜尋資料庫資料
			$sql = "SELECT * FROM user WHERE u_mail='$id' AND u_isroot='1'";
			...
			if($row[0] == $id && $row[1] == $pwd)
			{
				...
				$_SESSION['u_phone'] =$row[3];
       		 	header('Location:food_manage_lobby.php');
			}
      		else
      		{
       		 	header("Refresh:3; url=food_manage_login.php");
				$say = "登入失敗，三秒後跳回登入畫面.<BR>\n<a href='food_manage_login.php'>不想等待請按此</a>"; 
				echo $say;
			}
		?>
	</body>
